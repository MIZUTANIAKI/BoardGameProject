...
void ImageMng::DrawNaw(void)
{
	...
	for (auto dtxt : drawtextList_)
	{
		DrawFormatString(dtxt.first.x, dtxt.first.y, dtxt.second.first, dtxt.second.second.c_str());
	}
}
...
void ImageMng::PrintTextMestoDx(Vector2 pos, unsigned int col, std::string str)
{
	drawtextList_.emplace_back(std::pair<Vector2, std::pair<unsigned int, std::string>>(std::move(pos), std::pair<unsigned int, std::string>(std::move(col), std::move(str))));
}
